...
podTemplate(label: 'jenkins-pipeline', nodeSelector: 'os=linux', containers: [
    containerTemplate(name: 'jnlp', image: 'jenkins/jnlp-slave:3.10-1', args: '${computer.jnlpmac} ${computer.name}', workingDir: '/home/jenkins', resourceRequestCpu: '200m', resourceLimitCpu: '200m', resourceRequestMemory: '256Mi', resourceLimitMemory: '256Mi'),
    containerTemplate(name: 'docker', image: 'docker:stable-dind', command: 'cat', ttyEnabled: true),
    containerTemplate(name: 'golang', image: 'golang:1.9.2', command: 'cat', ttyEnabled: true),
